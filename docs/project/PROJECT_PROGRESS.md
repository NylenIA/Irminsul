...
| Team builder et simulation | 15 | 15 | 94 | medium | Team Lab CRUD + reactions + rotations + comparateur + recommandations ; PAGE /simulation : vraies simulations gcsim (E2E reelle) + SQUELETTE gcsim depuis equipe ENRICHI du scan GOOD : perso/arme/sets + STATS D'ARTEFACTS reelles (sumArtifactStats, main 5*niv20 table exacte + substats, cles/decimales gcsim, contribution artefacts sans double-comptage, gate strict sinon TODO) ; 16 persos du scan declenchent le calcul ; fonction pure testee (108 vitest) | rotation auto (laissee a l'utilisateur par design) ; energie/cooldown ; buffs par action |
