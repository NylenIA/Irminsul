...
| Application desktop | 10 | 12 | 55 | high | ADR runtime decide (Option A) + spike Next standalone PROUVE (server.js loopback HTTP 200) + mecanisme provenance sidecar. Coque Tauri+MSI/NSIS+sidecar existent | P0: coque enveloppe ancien Vite (pas encore le Next standalone) ; P0: binaire sidecar diverge (pas rotation/reactions) -> unifier+rebuild ; smoke packaging non fait |
